<?php
// export_excel.php — Export รายงานยอดขายเป็น Excel (หรือ CSV ถ้าไม่มี PhpSpreadsheet)
...
if (class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
  ...
  // ยังไม่ได้ติดตั้ง => ลอง auto-load ถ้ามี vendor
  $vendor = __DIR__ . '/../vendor/autoload.php';
  if (file_exists($vendor)) { require_once $vendor; }
}

// เช็คอีกทีหลัง autoload
if (class_exists('\PhpOffice\PhpSpreadsheet\Spreadsheet')) {
  ...
  // Header columns
  $ws->setCellValue('F3', 'จำนวน');
  $ws->setCellValue('G3', 'ราคาต่อหน่วย');
  $ws->setCellValue('H3', 'จำนวนเงิน');
  $ws->getStyle('F3:H3')->getFont()->setBold(true);
  $ws->getStyle('F3:H3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FFEEF5FF');
  ...
  $lastDataRow = $row;
  $ws->getStyle("E3:H{$lastDataRow}")
     ->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->getColor()->setARGB('FFBFD4FF');
  $ws->getStyle("F4:F{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
  $ws->getStyle("G4:H{$lastDataRow}")->getNumberFormat()->setFormatCode('#,##0.00');
...
fputcsv($out, []);
fputcsv($out, ['รวมทั้งหมด', '', '', '', number_format($grandAmount, 2)]);
fclose($out);
exit;
